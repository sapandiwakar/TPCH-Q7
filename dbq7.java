...
public class dbq7 {
  ...
  static Schema schemaImdN1Supplier_LineItem = schemaMgrN1Supplier.join(schemaLineItem);
  ...
  static Relation relCustomer = new Relation(schemaCustomer, inCustomer);

  // Intermediate result files
  static Relation relImdN1Supplier = new Relation(schemaMgrN1Supplier, tmpFilesPrefix
      + "imdSupplier");
  static Relation relImdN1SupplierLineItem = new Relation(schemaImdN1Supplier_LineItem,
      tmpFilesPrefix + "imdSupplierLineItem");
  ...
  public static JobControl buildJobs() throws IOException {
    ...
    // ======
    // map-side join o(n2) |><| supplier
    // =========
    // TODO: this still will have to know about selections and projections

    /*
     * JobConf job_n2_customers_conf = ReduceSideJoin.getConf(relCustomer,
     * relNations, "NATIONKEY", relImdN1Supplier);
     * SelectionFilter.addSelectionsToJob(job_n2_customers_conf,
     * ReduceSideJoin.PREFIX_JOIN_SMALLER, nationFilters, relNations.schema);
     * 
     * ControlledJob job_n2_suppliers = new
     * ControlledJob(job_n2_customers_conf); jbcntrl.addJob(job_n2_suppliers);
     */
    /**
